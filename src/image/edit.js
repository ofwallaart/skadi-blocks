...
export class ImageEdit extends Component {
	...
	onSelectImage(media) {
		...
		const { id, url, alt, linkDestination } = this.props.attributes;
	...
	render() {
		...
		const needsAlignmentWrapper = ["center", "left", "right"].includes(align);
				...
				<Fragment>
					...
					<div className={className}>
						{needsAlignmentWrapper ? (
							<div className={`align${align}`}>{mediaPlaceholder}</div>
						) : (
							mediaPlaceholder
						)}
					</div>
